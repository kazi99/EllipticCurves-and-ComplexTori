...
Projektivne transformacije tvorijo grupo za kompozitum, ki jo označujemo z $\operatorname{PGL(3, \C)}$, posebaj je $\Aut(\RS) \cong \operatorname{PGL(2,\C)}$. Več o tem lahko bralec najde v \cite[Poglavje 11]{Gibson}.
\kom{to lahko dokažem tudi kot trditev.}
...
\begin{definicija}
    ...
    $$ G = \lambda (F \circ \Phi). $$
    Pravimo, da sta tedaj tudi krivulji $V(F)$ in $V(G)$ \emph{projektivno ekvivalentni}.
\end{definicija}
\kom{pri tej definiciji je mogoče malce nedoslednosti v zapisu $F \circ \Phi$...}

Projektivno ekvivalenco dveh krivulj lahko interpretiramo možnost pretvorbe njunih minimalnih polinomov z uvedbo novih koordinat.
