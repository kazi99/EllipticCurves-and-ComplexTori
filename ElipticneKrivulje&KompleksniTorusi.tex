...
\begin{izrek}(Izrek o implicitni preslikavi)
    ...
    Naj bo $\Omega \subseteq \C^2$ odprto območje in naj bo $f : \Omega \to \C : (z, w) \mapsto f(z,w)$ funkcija, ki je holomorfna v obeh spremenljivkah posebej. Denimo, da je $(\alpha, \beta) \in \Omega$ ničla za $f$ in, da velja $\pdv[f]{w}(\alpha, \beta) \neq 0$, kjer $\pdv[f]{w}$ označuje kompleksni odvod po drugi spremenljivki. Tedaj obstajata dovolj majhni okolici $U \subseteq \C$ točke $\alpha$ in $V \subseteq \C$ okolica točke $\beta$ ter enolično določena holomorfna preslikava $\phi : U \to V$, ki izpolnjuje pogoj: 
...
\begin{dokaz}
    Na zvezo $f(z, \phi(z)) = 0$, ki velja povsod na $z \in U \subseteq \C$ delujemo s Caucy-Riemannovim operatorjem $\pdv{\bar{z}}$ in dobimo
    \[
        f_{\bar{z}}(z, \phi(z)) + f_{\bar{w}}(z, \phi(z)) \pdv[\phi]{\bar{z}}(z) = 0.  
    \] 
\end{dokaz}
