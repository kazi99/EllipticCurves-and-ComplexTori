...
Naslednja rezultat, ki ga bomo samo navedli brez dokaza, saj zanj potrebujemo pojme, katerih v nadaljevanju ne bomo obravnavali, zagotavlja, da se brez škode za splošnost -- pri obravnavi nesingularnih kubik -- omejimo samo na tiste v Weierstrassovi normalni formi. 

\begin{trditev}
    Vsaka nesingularna projektivna kubika je projektivno ekvivalentna neki nesingularni Weierstrassovi kubiki podani z enačbo
    \[
        C: \quad y^2z = x^3 + \alpha xz^2 + \beta z^3,
    \]
    kjer je ob tem vrednost  
\end{trditev}
...
\begin{definicija}
    ...
    \]
    skupaj s t.~i.\ \emph{izhodiščem} $O \in E$ na njej se imenuje \emph{eliptična krivulja} nad poljem $\C$. 
\end{definicija}

\begin{opomba}
    \begin{enumerate}
        \item 
        Zaradi kompletnosti smo v definicijo eliptične krivulje vklučili še izbiro izhodišča, ki igra vlogo identitete, ko eliptično krivuljo opremimo z grupno strukturo. Za lažje računanje se za izhodišče izbere enega od devetih prevojev, ki je najpogosteje kar točka v neskončnosti $\pcoor{0:1:0}$.  
        \item 
        Morda nepotrebno smo tukaj poudarjali, da je naša eliptična krivulja definirana nad poljem kompleksnih števil. V splošnem se elitične krivulje obravnava nad poljubnim poljem, kjer pride do izraza njegova karakterisika, ali je algebraično zaprto ipd. V našem primeru nad $\C$ takšnih skrbi ne bomo imeli. 
    \end{enumerate}
\end{opomba}
