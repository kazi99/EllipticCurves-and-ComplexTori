...
\povzetek{\emph{Eliptična krivulja} nad poljem kompleksnih števil $\C$ je nesingularna projektivna kubika, podana z Weierstrassovo enačbo. S pomočjo izreka o implicitni funkciji pokažemo, da ta dopušča kompleksno sturkturo, kar pomeni, da postane Riemannova ploskev. Po drugi strani vsak torus, predstavljen kot kvocient $\C$ po mreži $\Lambda$ -- diskretni podgrupi $\C$ izomorfni $\Z^2$, podeduje kompleksno strukturo preko kvocientne preslikave in tako pojasni imenovanje \emph{kompleksni torus}. Izkaže se, da ti dve, bistveno različni konstrukciji, presenetljivo porodita enaka \oz izomorfna matematična objekta. To vez razložimo preko teorije dvojno periodičnih funkcij, imenovanih eliptične funkcije, in modularnih funkcij z mnogo simetrije, povezane z modularno grupo $\operatorname{SL}_2(\Z)$. Obravnavamo njihove osnovne lastnosti in razvijemo teorijo Weierstrassove funkcije $\wp$, ki nam nazadnje omogoči eksplicitno podati omenjeni izomorfizem in dokazati uniformizacijski izrek, ki združuje oba objekta.
} 
...
% če bi hoteli pogoj, da je neki "vodilni" koeficient enak 1, bi potebovali uvesti neko monomsko ureditev
...
\begin{izrek}
    ...
    % \begin{enumerate}[(i)]
    %     \item \[\sum_{w \in D} \res{w}{f} = 0\] \label{eq: identiteta 1}
    %     \item \[\sum_{w \in D} \ord{w}{f} = 0\] \label{eq: identiteta 2}
    %     \item \[\sum_{w \in D} \ord{w}{f}\cdot w \in \Lambda.\] \label{eq: identiteta 3}
    % \end{enumerate}    
\end{izrek}
...
\begin{dokaz}
    ...
    % \begin{enumerate}
        ...
        (ii) $p = \pcoor{0 : 1 : 0}$: Za točko v neskončnosti imamo na obeh krivuljah lokalni karti $\varphi$ okrog $p$ in $\psi$ okrog $g(p) = \oio$ podani s predpisoma 
        \[
        ...
        njuna inverza pa z 
        \[
            \varphi\inv(w) = \pcoor{w : 1 : \phi_1(w)} \quad \text{in} \quad
            \psi\inv(w) = \pcoor{w : 1 : \phi_2(w)},
        \]
        ...
        \begin{align*}
            \psi(g(\varphi\inv(w))) &= 
            \psi(g(\pcoor{w : 1 : \phi_1(w)})) \\
            &= \psi(\pcoor{u^2w : u^3 : \phi_1(w)}) =
            \psi\left(\pcoor{\tfrac{w}{u} : 1 : \tfrac{\phi_1(w)}{u^3}}\right) = 
            \frac{w}{u}.
        \end{align*}
        Slednji račun pokaže, da je $\psi \circ g \circ \varphi\inv$ na tej odprti okolici holomorfna.
        % \item[(iii)]

        (iii) Nazadnje naj bo točka $p \in E_1$ poljubna, ki ni iz zgornjih dveh primerov. Tedaj imamo okoli $p$ in $g(p)$ lokalni karti $\varphi$ in $\psi$ s predpisoma
        \[
        ...
        \]
        kjer sta $\phi_1$ in $\phi_2$ ustrezni holomorfni funkciji, katerih graf je lokalno afini del krivulj $E_1$ \oz $E_2$ okoli točk $p$ \oz $g(p)$. Tedaj izračunamo
        \[
            \psi(g(\varphi\inv(w))) =
...
\begin{dokaz}
    ...
    \[
        \Im(\tau_1) = 
        % \Im\left(\frac{\alpha\tau_1}{\alpha}\right) =
...
\begin{dokaz}
    ...
    \]
    če za $\delta$ vzamemo $\delta := \min\left\{\delta_{a + i\varepsilon}, \delta_{-a + i\varepsilon}, \frac{\varepsilon}{\abs{i - a}}\right\} \in (0,1)$, kar zaključi dokaz. \qedhere 
...
\begin{komentar}
    ...
    % O tem lahko več izvemo v \cite[VII, \S3]{Serre}.
\end{komentar}
...
%     zaradi $\wp'(z) \in \CM$ za vse $z \in \C \setminus \tfrac{1}{2}\Lambda$. Tako vidimo, da je $\phi$ zvezna preslikava.
    ...
    \begin{itemize}
        ...
        \]
        % Ker je $\wp$ soda in reda $2$, iz $\wp(z_1) = \wp(z_2)$ sledi
        % \[
