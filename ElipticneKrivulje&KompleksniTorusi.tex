...
\]
projektivno ekvivalentni, če le velja $u^4 \alpha' = \alpha$ in $u^6 \beta' = \beta$ za nek $u \in \CM$. Namreč takrat imamo projektivnost
\begin{align*}
    \Phi:C &\to C'\\
...
Ugotovili smo, da lahko dva različna para koeficientov $\alpha, \beta \in \CM$ podata projektivno ekvivalentni Weierstrassovi kubiki. Obstaja pa količina, ki se pri tovrstnih transformacijah ne spreminja -- ostaja invariantna. Tej količini pravimo \emph{$j$-invarianta} Weierstrassove kubike, oziroma pozneje, eliptične krivulje. Podana je kot 
\[
    j = 1728(4\alpha)^3/\Delta = -1728\frac{4\alpha^3}{4\alpha^3 + 27\beta^2}.  
\] 
Jasno je, da se pri transformaciji \ref{} od prej $j$-invarianta ohranja. Krajši račun pokaže
\[
...
\begin{definicija}
    Nesingularna projektivna kubika $E(\C)$ ali samo $E$ podana z enačbo v Weierstrassovi obliki
    \[
      E: \quad y^2z = x^3 + \alpha xz^2 + \beta z^3,
    \]
    skupaj s t.~i.\ \emph{izhodiščem} $O \in E(\C)$ na njej se imenuje \emph{eliptična krivulja} nad poljem $\C$. 
\end{definicija}

\begin{opomba}
    \begin{enumerate}
        \item 
        Zaradi kompletnosti smo v definicijo eliptične krivulje vklučili še izbiro izhodišča, ki igra vlogo identitete, potem ko eliptično krivuljo opremimo z grupno strukturo. Za lažje računanje se za izhodišče izbere enega od devetih prevojev, ki je najpogosteje točka v neskončnosti $\pcoor{0:1:0}$.  
        \item 
...
V nadaljevanju bo ugodneje namesto \emph{klasične} Weierstrassove oblike nesingularne kubike \ref{klasicna wnf} obravnavati malenkost prilagojeno -- projektivno ekvivalentno -- različico
\[
