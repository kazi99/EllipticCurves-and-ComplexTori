...
\begin{dokaz}
    ...
    % \end{pmatrix},
    % $$
    \noindent kjer smo v drugi enakosti po $2 \times 2$ blokih upoštevali Caucy-Riemannov sistem enačb, saj imamo opravka s polinomi, ki so kot funkcije holomorfni v obeh svojih kompleksnih spremenljivkah. Izračun
    \[
...
\begin{opomba} 
    ...
    \[
        q : \C^{n+1}\setminus\{0\} \to P^n(\C) \quad (x_0, \ldots, x_n) \mapsto \pcoor{x_0 : \ldots : x_n}. 
    \]
    In sicer vzamemo za odprte množice v $P^n(\C)$ \emph{natanko} tiste $U \subseteq P^n(\C)$ za katere je $q\inv(U)$ odprta v $\C^{n+1}\setminus\{0\}$. Hkrati je to tudi največja topologija na $P^n(\C)$, za katero je projekcija $q$ še vedno zvezna.  
...
Pozneje bomo videli, kako lahko $j$-invarianto gledamo tudi kot funkcijo kompleksne spremenljivke in tako malce pokomentirali ``zanimivost'' izbire faktorja $1728$ pred celotno formulo.
\\
...
\begin{dokaz}
    ...
    Z menjavo spremenljivk $w = z + \om_1$ v prvem in $w = z + \om_2$ v četrtem interalu je zaradi periodičnosti $f$ in orientacije obeh parov nasprotnih stranic ($\sigma_1$ in $\sigma_3$ ter $\sigma_2$ in $\sigma_4$) razvidno, da se integrala po parih nasprotnoležečih stranic izničita, kar nam da želeni rezultat $0$.
    \\
    \par
...
\]
Zaradi absolutne konvergence te vrste po lemi \ref{lema o absolutni in enakomerni konvergenci} lahko predstavimo, da jo seštevamo postopoma po diskih $\disk{0}{n}$ za $n\in\N$ in tako dobimo zaporedje delnih vsot, ki so holomorfne na $\C\setminus\Lambda$ in konvergirajo k $f$. Po izreku \ref{izrek o konvergenci holomorfnih funkcij} je $f$ holomorfna na $\C \setminus \Lambda$, v vsakem $\om_0 \in \Lambda$ pa ima pol reda $k$ in residuujem $0$. O tem se prepričamo, saj lahko zapišemo
\[
    f(z) = \frac{1}{(z-\om_0)^k} + \sum_{\om \in \Lambda \setminus \{\om_0\}} \frac{1}{(z - \om)^k}
\]
na neki dovolj majhni prebodeni okolici točke $\om_0$. Glavnemu delu okrog $\om_0$ prispeva samo člen $(z - \om_0)^{-k}$, vrsta, ki ostane, pa je zaradi leme \ref{lema o absolutni in enakomerni konvergenci} po podobnem razmisleku kot zgoraj holomorfna na tej prebodeni okolici in zato na glavni del nima vpliva. Tako je $f$ meromorfna funkcija na $\C$. 
...
En način, kako bi se tej izbiri lahko izgodili, je, da njuni domeni zamenjamo s krivuljo $X = \{(z,w) \in \C^2 \mid w^2 = z\}$, ki je v bistvu unija grafov obeh vej, kvadratni koren pa interpretiramo kot projekcijo iz $X$ na $w$-os, \tj $(z,w) \mapsto w$. Tako se izognemo tej nekanonični izbiri veje, a smo s tem zašli v nov problem. Izgubili smo poznano predstavo holomorfnih funkcij na tej domeni, ki sedaj ni več neka odprta množica v $\C$. Na srečo pa imamo še vedno upanje povrniti to informacijo, saj lahko množico $X$ lokalno sploščimo in identificiramo z neko odprto množico v $\C$. Če znamo takšne lokalne identifikacije najti na celotnem $X$ in to storimo na nek usklajen način, bo nastala struktura dopuščala razviti definicijo, ki opredeljuje holomorfne funkcije na $X$, in dopuščala adaptirati določene izreke o odprtih domenah v $\C$ na te abstraktne objekte, ki jih imenujemo \emph{Riemannove ploskve}.  
...
\]
vložitev, $q : \C^3 \setminus \{0\} \to \PC$ pa kvocientna projekcija iz opombe \ref{topologija na projektivnih prostorih}, kjer smo projektivne prostore opremili s topologijo. Najprej opazimo, da se projektivno zaprtje $\olsi{E} \subseteq \PC$ od afine krivulje $E$ v bistvu razlikuje samo v eni točki. Natančneje vidimo, da je 
\[
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \]
        Ob tem sta $\phi_1, \phi_2$ holomorfni funkciji, ki v konstrukciji kompleksnega atlasa izhajata iz primera, ko lahko afina dela krivulj $E_1$ in $E_2$ okoli $p$ oziroma $g(p)$ izrazimo kot grafa funkcij $\phi_1$ in $\phi_2$ nad spremenljivko $y$. Tedaj izračunamo
        \[
            \psi(g(\varphi\inv(w))) = 
...
Zadnja opomba in zgled \ref{meromorfne so holomorfne v R.sfero} nam omogočata eliptične funkcije pogledati še z vidika Riemannovih ploskev. Eliptična funkcija $f \in \C(\Lambda)$ je meromorfna funkcija na $\C$, ki jo po zgledu lahko realiziramo kot holomorfno funkcijo $\C \to \RS$. Poleg tega je po definiciji njena vrednost dobro definirana na translatih oblike $z + \Lambda$, torej inducira zvezno preslikavo na kvocientu $\torus \to \RS$, ki pa ni le zvezna, ampak tudi holomorfna.  Ekvivalentno lahko torej rečemo, da so eliptične funkcije glede na mrežo $\Lambda$ natanko holomorfne funkcije $\torus \to \RS$. Imamo bijektivno korespondenco množic
\[
...
\begin{zgled*}
    ...
    \]
    Naj bo $f : \C \to \C$ funkcija podana s predpisom $f(z) = az + b$ in označimo kvocientni projekciji $\pi_1 : \C \to \C/\Lambda_1$ ter $\pi_2 : \C \to \C/\Lambda_2$. 

    Zaradi pogoja $a\Lambda_1 \subseteq \Lambda_2$, za poljubni točki $z, w \in \C$, za kateri je $\pi_1(z) = \pi_1(w)$ \oz $z + \Lambda_1 = w + \Lambda_1$, sledi tudi $\pi_2(f(z)) = \pi_2(f(w))$ \oz $az + b + \Lambda_2 = az + b + \Lambda_2$.
    ...
    Od tod se po izreku \cite[]{MrcunTop} inducira zvezna preslikava $\olsi{f} : \C/\Lambda_1 \to \C/\Lambda_2$, da velja $\olsi{f} \circ \pi_1 = \pi_2 \circ f$ oziroma, da komutira naslednji diagram.
    \[
        \begin{tikzcd}
            {\mathbb{C}} & {\mathbb{C}} \\
            \\
    ...
    \]
    holomorfna preslikava na okolici $U$. Pokazali smo, da je $\olsi{f}$ holomorfna na neki odprti okolici vsake točke iz $\C/\Lambda_1$, zato je holomorfna tudi kot preslikava $\olsi{f}: \C/\Lambda_1 \to \C/\Lambda_2$. Po tej konstrukciji lahko zapišemo predpis inducirane preslikave $\olsi{f}$:
    \[
        z + \Lambda_1 \mapsto az + b + \Lambda_2.  
    \]
\end{zgled*}

V posebnem, ko vzamemo število $a \in \CM$ takšno, da velja celo enakost $a\Lambda_1 = \Lambda_2$, bo inducirana preliskava $\olsi{f} : \C/\Lambda_1 \to \C/\Lambda_2$, podana s predpisom $\olsi{f}(z + \Lambda_1) = az + b + \Lambda_2$, biholomorfizem. Njen inverz bo preslikava s predpisom $z + \Lambda_2 \mapsto \frac{z}{a} - \frac{b}{a} + \Lambda_1$, ki je tudi holomorfna, saj jo inducira inverz funkcije $f$, ki je tudi zahtevane oblike s predpisom $f\inv(z) = \frac{z}{a} - \frac{b}{a}$.
...
\begin{trditev}
    ...
    Ob tem je $f$ enolično določena s $\olsi{f}$, do prištevanja konstante iz $\Lambda_2$ natančno. 
        \item
    Če je $\olsi{f}$ tudi holomorfna, je $f$ holomorfna in oblike $f(z) = az + b$ za neka $a, b \in \C$ in ob tem posledično velja $a\Lambda_1 \subseteq \Lambda_2$ ter $\olsi{f}(z + \Lambda_1) = az + b + \Lambda_2$.   
    \end{enumerate}
\end{trditev}

\begin{dokaz}
    (i) Dokaz obstoja preslikave $f : \C \to \C$ se dotakne teorije krovnih prostorov, ki izkoristi enostavno povezanosti $\C$ ter lepo strukturo kvocientne projekcje $\pi_2$, in je ne bomo razvijali, zato se skličimo na \cite[Lema 3.1]{Stevenhagen}.
    ...
    \] 
    kar pomeni, da $g_\om$ slika v diskretno mrežo $\Lambda_2$ in je tako lahko le konstantna. Njen odvod je torej ničelni, torej za vse $z \in \C$ velja $f'(z + \om) = f'(z)$. Ker je bila perioda $\om \in \Lambda_1$ poljubna, sledi, da je $f'$ eliptična glede na $\Lambda_1$. Kot holomorfna eliptična funkcija, pa je ponovno lahko le konstanta po trditvi \ref{cele el. funkcije}. Tedaj je $f' \equiv a$, za neki $a \in \C$ in posledično za vse $z \in \C$ velja $f(z) = az + b$, za neki $b \in \C$, kar smo hoteli pokazati. 
\end{dokaz}
...
Nadalje se bomo posvetili mrežam v kompleksni ravnini in si ogledali kako smemo dano mrežo transformirati, da še vedno ostanemo v istem izomorfnostnem razredu kompleksnih torusov, ki ju dani mreži porodita. Ena od teh podob mreže bo prav posebne oblike, kar bomo izkoristili za nekoliko drugačno interpretacijo Eisensteinovih vrst iz \ref{elipticne funkcije}.\ poglavja, in sicer kot holomorfno funkcijo zgornje polravnivne. Od tod bomo to analogijo nadaljevali še z $j$-invarianto, kjer se bomo dotaknili \emph{modularnih funkcij}, kar nam bo nazadnje omogočilo vse skupaj povezali v uniformizacijskem izreku. 
% nazadnje pa bomo zaključili z uniformizacijskim izrekom. 

\subsection{Izomorfizem \texorpdfstring{$\C/\Lambda \to E(\C)$}{}} \label{poglavje izomorfizem}
...
\begin{dokaz}
    Razdelimo dokaz trditve na naslednje štiri dele: dobra definiranost, zveznost, bijektivnost in holomorfnost preslikave $\phi$.
    ...
    \[
        ...
        \end{cases}, 
    \]
    zato bo zaradi trditve \cite[]{MrcunTop} dovolj preveriti zveznost $\Phi$, ker je $\pi$ kvocientna. Podali bomo dva predpisa za $\Phi$, definirana na odprtih podmnožicah v $\C$, katerih unija bo pokrila $\C$, in oba predpisa se bosta na njunem preseku ujemala. 
...
V zaključku \ref{algebraicne krivulje}.\ poglavja smo spoznali, \ti $j$-invarianto eliptične krivulje, ki se je izkazala za popolno invarianto na prostoru eliptičnih krivulj do projektivnosti natančno. Drugače rečeon sta eliptični krivulji, z enako $j$-invarianto, nad poljem kompleksnih števil $\C$ vedno projektivno ekvivalentni in celo vsaki vrednost $j \in \C$ pripada neki razred projektivno ekvivalentnih eliptičnih krivulj. 
...
\subsubsection*{Mreže}
\begin{lema}
    Eisensteinova vrsta reda $k > 2$ za mrežo $\Lambda_\tau = \langle 1, \tau \rangle$
    \[
        G_k(\Lambda) = G_k(\langle 1, \tau \rangle) = 
        \sum_{(m,n) \in \Z^2\setminus\{0\}} \frac{1}{(m + n\tau)^k}  
    \]
    podaja holomorfno funkcijo na zgornji polravnini $\tau \in \HH$, ki jo ponovno označimo z $G_k : \HH \to \C$, $G_k(\tau) = G_k(\Lambda_\tau)$.
\end{lema}

\begin{dokaz}
    Zaradi Weierstrassovega M-testa in izreka \ref{izrek o konvergenci holomorfnih funkcij} zadošča pokazati, da je $G_k$ omejena po kompaktih v $\HH$. Če je $K \subseteq \HH$ poljuben kompakt, obstajata taka $a, \varepsilon > 0$, da je $K$ vsebovan v pravokotniku $S_{a,\epsilon} = \{z \in \HH \mid \abs{\Re(z)} \leq a \text{, }\varepsilon \leq \Im(z) \leq a\}$. Izkaže se, da obstaja tak $\delta \in (0,1)$, za katerega je
    \begin{equation}
        \label{eq: ocena za holomorfnost eisensteinove vrste}
        \abs{m + n\tau} \geq \delta\abs{m + ni},  
    \end{equation}
    za vse $(m, n) \in \Z^2\setminus\{0\}$ in vse $\tau \in S_{a,\varepsilon}$. To oceno uporabimo na kompaktu $K$ za majorizacijo $G_k(\tau)$ na sledeč način
    \[
    ...
    Vrnimo se sedaj še k oceni \ref{eq: ocena za holomorfnost eisensteinove vrste}. Če je $n = 0$, ocena drži za katerikoli $\delta$, zato bo ekvivalentno pokazati obstoj takega $\delta \in (0,1)$, da bo za vse $(m,n) \in \Z^2$, kjer $n \neq 0$, in $\tau \in S_{a,\varepsilon}$ veljalo
    \[
        \abs{\frac{\tau + \tfrac{m}{n}}{i + \tfrac{m}{n}}} \geq \delta.
    \]
    Definirajmo zvezno funkcijo $f: S_{a, \varepsilon} \times \R \to (0,\infty)$ s predpisom $f(\tau, x) = \abs{\tfrac{\tau - x}{i - x}}$. Za vsak fiksen $\tau$, velja $\lim_{x \to \pm\infty} f(\tau, x) = 1$, torej obstaja $R_\tau > 0$, da je $\abs{\tfrac{\tau - x}{i - x}} \geq \tfrac{1}{2}$ za vse $\abs{x} \geq R_\tau$. Hkrati pa zaradi zveznosti, funkcija $f$ na kompaktu $\{\tau\} \times [-R_\tau, R_\tau]$ doseže minimum $c_\tau > 0$. Tedaj zadošča vzeti $\delta_\tau = \min\{c_\tau, \tfrac{1}{2}\}$, za katerega je $f(\tau, x) \geq \delta_\tau$.

    To oceno pa lahko izpeljemo tudi enakomerno glede na $\tau$. Za poljuben $\tau \in S_{a, \varepsilon}$ in $x > a$ velja 
    \[
        \abs{\frac{\tau - x}{i - x}} \geq \abs{\frac{a + i\varepsilon - x}{i - x}},
    \]
    kot vidimo na sliki ...., torej bo za vse $\tau \in S_{a, \varepsilon}$ in vse $x > \max\{R_{a + i\varepsilon}, a\} = r_+$ veljalo $\abs{\tfrac{\tau - x}{i - x}} \geq \tfrac{1}{2}$. Simetrično lahko sklepamo, da bo $\abs{\tfrac{\tau - x}{i - x}} \geq \tfrac{1}{2}$ za vse $\tau \in S_{a, \varepsilon}$ in vse $x < \min\{-R_{-a + i\varepsilon}, -a\} = r_-$. Zaradi zveznosti, $f$ doseže minimalno vrednost $c > 0$ na kompaktu $S_{a, \varepsilon} \times [r_-, r_+]$, torej nazadnje, za $\delta = \min\{c, \tfrac{1}{2}\} > 0$ dobimo 
    \[
...
\begin{trditev}
    Dve mreži sta si podobni natanko tedaj, ko sta njuni $j$-invarianti enaki. 
\end{trditev}
...
\begin{izrek}
    ...
    \[
        E: \quad y^2z = 4x^3 - axz^2 - bz^3; \quad a^3 - 27b^2 \neq 0,  
    \]
    obstaja mreža $\Lambda \subseteq \C$, da je $\torus \cong E(\C)$ izomorfizem Riemannovih ploskev. 
\end{izrek}

\begin{dokaz}
    Naj bo $j_E = 1728\frac{a^3}{a^3 - 27b^2}$ $j$-invarianta eliptične krivulje $E$. Tedaj po izreku \ref{bijektivnost j-invariante} obstaja tak $\tau \in \HH$, da je $j(\tau) = j_E$. Za mrežo $\Lambda = \langle 1, \tau \rangle$ tedaj velja   
\end{dokaz}
...
\begin{dokaz}
    Naj bosta $E_1$ in $E_2$ eliptični kriulji s pripadajočima kompleksnima strukturama. Če sta projektivno ekvivalentni, nam trditev \ref{projektivnost je biholomorfizem} pove, da je projektivnost med njima holomorfna preslikava s holomorfnim inverzom, kar pomeni, da je biholomorfizem med $E_1$ in $E_2$.

    Obratno, denimo, da sta $E_1$ in $E_2$ izomorfni kot Riemannovi ploskvi. Tedaj po uniformizacijskem izreku \ref{uniformizacija} dobimo mreži $\Lambda_1$ in $\Lambda_2$, da sta $\C/\Lambda_1 \iso E_1$ in $\C/\Lambda_2 \iso E_2$ izomorfizma Riemannovih ploskev, in posledično izomorfizem $\C/\Lambda_1 \iso \C/\Lambda_2$. Po trditvi \ref{oblika holomorfnih preslikav med torusi} v posebnem dobimo, da je $\alpha\Lambda_1 = \Lambda_2$ za neki $\alpha \in \CM$, od tod pa 
\end{dokaz}
