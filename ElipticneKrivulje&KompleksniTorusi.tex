...
\begin{dokaz}
    ...
    \] 
    Poglejmo si sedaj zadnji integral, ki ga podobno kot pri dokazu (\ref{identiteta 1}) razbijemo na vsoto integralov po štirih stranicah. Z uvedbo nove spremenljivke $w = z + \omega_2$ v integral po stranici $\sigma_1$ vidimo
    \begin{multline*}
    ...
    Za poljubno sklenjeno krivuljo $\gamma : [0,1] \to \C$, tj. $\gamma(0) = \gamma(1)$, ki ne poteka skozi izhodišče $0\in \C$, je
    \[
        \frac{1}{2 \pi i} \int_\gamma \frac{dz}{z} \in \Z
    \]
    \emph{ovojno število} krivulje $\gamma$ okoli $0$. Več o tem lahko bralec najde v \cite[4.2.1.]{Ahlfors}.
    \\

    Osredotočimo se sedaj samo na prvi integral, premislek za drugega je analogen. Opazimo, da je zaradi eilptičnosti $f$ krivulja $f(\sigma_4)$ sklenjena, saj sta krajišči daljice $\sigma_4$ točki $\alpha$ in $\alpha + \omega_2$ v katerih ima $f$ enaki vrednosti. Pot $\gamma : [0,1] \to f(\sigma_4)$, ki predstavlja to sklenjen0 krivuljo, je podana s predpisom $t \mapsto f(\alpha + t\omega_2)$.\kom{to ni parametrizacija krivulje v običajnem smislu, saj je lahko neinjektivna, je potrebno kaj takega še omeniti?} Torej lahko zapišemo 
    \[
        2 \pi i k_1 = 
...
Osrednja tema tega poglavja, ki bo povezala eliptične krivulje s kompleksnimi torusi in za katero smo potrebno teorijo razvijali v prejšnjem razdelku, bo t.~i.\ \emph{Weierstrassova funkcija} $\wp$. 


Vseskozi naj bo $\Lambda$ mreža v $\C$ in naj velja oznaka $\Lambda' = \Lambda \setminus \{0\}$.

\begin{lema}
    \label{lema o oceni vrste po mrezi}
    Za vsako celo število $k > 2$ je vrsta 
    \[
        \sum_{\om \in \Lambda'} \frac{1}{\om^k}
    \]
...
\begin{definicija}
    ...
    Za celo število $k > 2$ je \emph{Eisensteinova vrsta reda $k$}
    \[
...
\begin{izrek}
    o konvergenci holomorfnih funkcijskih vrst
\end{izrek}
...
\begin{lema}
    ...
    \]
    konvergira absolutno za poljuben $z\in \C\setminus\Lambda'$ in enakomerno po kompaktih na $\C \setminus \Lambda'$. Kot preslikava $\C \to \RS$ pa predstavlja meromorfno funkcijo s poli v možici $\Lambda'$. 
\end{lema}
